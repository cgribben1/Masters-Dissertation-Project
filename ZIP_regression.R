...
# Create the ZIP regression model
zipmodel <- zeroinfl(count ~ hour + district + month + weekday, data = chic, dist = "poisson")

summary(zipmodel)

# Use model to predict crime count
predict(zipmodel, newdata = data.frame(hour = factor(1), district = factor(1), month = factor(7), weekday = factor(5)), type = "response")
