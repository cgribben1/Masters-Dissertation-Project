...
# Create the ZINB regression model
zinbmodel <- zeroinfl(count ~ hour + district + month + weekday, data = chic, dist = "negbin")

summary(zinbmodel)

# Use model to predict crime count
predict(zinbmodel, newdata = data.frame(hour = factor(2), district = factor(1), month = factor(7), weekday = factor(5)), type = "response")
